...
- **次のステップ**:
  - **200語への拡張(v0.3相当)はユーザー判断で先送り**。pipe-v2基準測定は完了(ミラー拡張ありでも top-1 27-29%/top-5 51-65%、基準の70%/90%に届かず)。次のレバー(テイク数3→5、モデルスケールアップ、語彙整理、ミラー拡張を本番モデルに反映するか)の選択は未確定
