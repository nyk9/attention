...
- **ステータス**: ①プロンプト試作完了(2026-07-02)。①プロンプト→②LoRA→③自作の①段階で、コードは無くプロンプト(`prompt_v1.md`)と検証記録(`eval_v1.md`)のみ
- **検証所見**: 1語素通し・過剰生成抑止・不確実性明示(?)は動く。文脈による top-k 挽回は「top-1 の組が意味的に破綻」時のみ発動し、現語彙50語では機会が少ない → **②への投資判断は文(複数サイン)認識が動いてからで十分**。高確信度の単発誤り(左右手ゆれ由来)は部品Cでは救えない(認識側の課題との責任分界を確認)
- **注意**: 検証はセッション内スモークのみで API 独立実行は未実施。プロンプト内語彙表は words.tsv と要同期(50語収載。word_id 022 は 2026-07-02 に tatoeba/例えば に確定)
...
- **stage 2 撮影・評価 完了(2026-08-01)**:
  ...
  - **`--eval-holdout`(既定: テイク3を未見)は 41語中 39語で top-1 5%(2/41)・top-5 24%(10/41)** と stage 1(30%/80%)から大幅悪化。**原因を特定**: `--inspect-dict` で確認したところ、**41語中34語(83%)が「テイク1・2=右手、テイク3=左手」という同一パターン**(ユーザーへの聞き取りで撮影時の癖と確認)。`--eval-holdout` の既定分割(テイク番号の後ろ1件を毎回未見にする)と噛み合い、ほぼ全語で「学習=右手のみ、未見=左手のみ」という体系的なズレが発生し、この評価では正しく汎化性能を測れていない。**モデルの学習能力不足という証拠ではない**(左右手ゆれの根本原因は既知。[[stage1-hand-switching-finding]] 参照)
...
- **次のステップ**:
  - **200語への拡張(v0.3相当)はユーザー判断で先送り**。次に何をやるかは未確定(pipe-v2基準測定 = 50語でtop-1 70%/top-5 90%を測る、ミラー拡張を本番モデルに反映するか、など候補はあるが決定していない)
  - タグ → 自然な日本語(部品C)は LLM 整形を後段で(①プロンプト→②LoRA→③自作の順で安く試す)。**①の試作は完了**(`llm_shaping/` 参照)。実 top-k での API 独立実行の再検証は未着手のまま
