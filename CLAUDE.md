...
- **注意**: プロンプト内語彙表は words.tsv と要同期(51語収載、2026-08-14に stage 2 の語彙整理分を同期。word_id 022 は 2026-07-02 に tatoeba/例えば に確定)
...
- **次のステップ**:
  - **200語への拡張(v0.3相当)に進むと決定(2026-08-14)**。pipe-v2基準測定は完了(ミラー拡張ありでも top-1 27-29%/top-5 51-65%、基準の70%/90%に届かず)、原因診断の結果「モデル容量に対しサンプル数が極端に少ない(パラメータ/サンプル比≈2,400倍)」ことが主因と判明。テイク数3→5だけでは基準到達は見込み薄いと分かった上で、**pipe-v2基準は今は追わず、新規語から1単語5テイクにしつつ単語数も増やす**方針(v1.0の「広げて後で絞る」設計思想と整合)。**ろう者協力者レビューは前提から外し先に進める**(協力者の反応が芳しくないため、2026-08-14ユーザー決定)。詳細は `docs/development-plan.md` 参照
