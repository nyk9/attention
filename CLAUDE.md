...
- **注意**: 検証はセッション内スモークのみで API 独立実行は未実施。プロンプト内語彙表は words.tsv と要同期(word_id 022 は `???` 未決定のため未収載)
...
- **左右手ゆれの真因判明+診断ツール `--inspect-dict` 追加(2026-07-02)**:
  ...
  - あわせて words.tsv の採用語彙をユーザーが修正(手話で同一表現の語の統合=are-sore、別表現になる語の分割=kare/kanojo など。撮影済み 001〜010 は不変)。**注意: word_id 022 が `???` プレースホルダのまま**(テンプレは maamaa。stage 2 撮影がこの語に到達する前にユーザーが決める)
