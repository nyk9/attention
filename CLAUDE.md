...
- **次のステップ**:
  - stage 2(words.tsv の11-20語目)を撮影 → 同じ手順(build-dict 用にフラット整理 → `--eval-holdout`)で評価し、左右手ゆれが再発するか確認
