...
- **追加機能(複数選択可)**:
  ...
  - **MediaPipe Hands (Palm + Landmark) を並走**: 手指検出を並列実行
    ...
    - 選択すると「**Palm 検出に渡す領域**」を追加で質問される(2026-08-22 追加)
- **Palm 検出に渡す領域**(上記「Hands を並走」を選んだ場合のみ):
  - **フレーム全体(従来・既定)**: フレーム全体を 192x192 にレターボックス。
    1620x1080 では入力の 33% が黒帯になり、原寸 217px(実測中央値)の手が約 25.7px になる
  - **中央正方クロップ(改善案 A)**: 短辺の正方を中央から切る。実測では**検出率の改善はゼロ**で、
    検出済みの手の 3.7% を横方向に切り落とす。**ただし下流の認識精度は最も良かった**
    (実験5、10 seed: top-1 68.8% / top-5 88.4%。full-frame 58.6% / 82.5%、
    tiles3 66.1% / 88.6%)。Palm 推論は 1 回のまま。詳細は
    `docs/experiments/person-detection-roi.md`
  - **実測サイン領域**: 撮影データの手の分布(x 48..1485 / y 247..1123)から決めた矩形。
    手は横方向にほぼ全幅を使うため効果は小さい
  - **正方タイル 3 枚**: 長辺方向に重なり付きの正方タイルへ分割し、結果をまとめて NMS。
    実効解像度が 1.5 倍になり黒帯が消える。**2 手同時検出が 19.6% → 24.5% に改善**する一方、
    推論コストは 3 倍。1 手検出はほぼ変わらない(詳細は `docs/experiments/palm-detection-roi.md`)。
    **下流の認識精度では中央正方クロップと差が無く(実験5)、3 倍のコストに見合う差は観測されていない**
  - **人物検出で切り出す(実験5)**: `models/person_detection_mediapipe_2023mar.onnx`
    (OpenCV Zoo、Apache-2.0、git 管理外なので別途ダウンロードが必要)で人物を検出し、
    MediaPipe と同じ式で作った人物 ROI を Palm に渡す。Palm 推論は 1 回のままだが
    人物検出 1 回ぶんのコストが増える。**現行の撮影データ(上半身のみが写る画角)では
    人物 ROI がフレームより大きくなり、短辺で頭打ちして実質「追従する中央正方クロップ」
    に縮退する**(詳細は `docs/experiments/person-detection-roi.md`)。
    人物が検出できなかったフレームはフレーム全体へフォールバックする
